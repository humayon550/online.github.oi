...
if($_GET['d'])
{
curl_setopt($ch, CURLOPT_URL, 'http://hindisongs.fusionbd.com/downloads/mp3_index.php?'.$_SERVER['QUERY_STRING'].'');
}
else
{
curl_setopt($ch, CURLOPT_URL, 'http://hindisongs.fusionbd.com/downloads/mp3_index.php?'.$_SERVER['QUERY_STRING'].'');
...
$page = curl_exec($ch);
curl_close($ch);
$page = preg_replace("|mp3_index.php|","hindi.php",$page);
...
$page = preg_replace('|p=2|','p=1',$page);
echo $page;
echo $ads;
?>