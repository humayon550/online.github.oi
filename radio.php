...
if($_GET['d'])
{
curl_setopt($ch, CURLOPT_URL, 'http://radiospecials.fusionbd.com/index.php?'.$_SERVER['QUERY_STRING'].'');
}
else
{
curl_setopt($ch, CURLOPT_URL, 'http://radiospecials.fusionbd.com/index.php?'.$_SERVER['QUERY_STRING'].'');
...
$page = curl_exec($ch);
curl_close($ch);
$page = preg_replace("|index.php|","radio.php",$page);
...
$page = preg_replace('|p=0|','p=2',$page);
$page = preg_replace('|p=1|','p=2',$page);
echo $page;
echo $ads;
?>